...
class InsightAgent:
    ...
    async def process(self, state: dict) -> dict:
        ...
        if not transcript_text:
            ...
            state["insights"] = {"meeting_id": meeting_id}
            return state
        try:
            ...
            logger.info(f"[InsightAgent] Analysis complete: sentiment={state['insights']['overall_sentiment']}, efficiency={state['insights']['efficiency_score']:.1f}")
        except Exception as e:
            logger.error(f"[InsightAgent] Error: {e}")
            state["errors"] = state.get("errors", []) + [f"InsightAgent: {str(e)}"]
            speaker_stats = self._compute_speaker_stats(transcript)
            state["insights"] = {"meeting_id": meeting_id, "speaker_stats": speaker_stats}
        return state

    @staticmethod
    def _compute_speaker_stats(transcript: Any) -> list[dict]:
    ...
    async def _analyze_with_llm(self, transcript_text: str, speaker_stats: list[dict]) -> dict[str, Any]:
        stats_text = "\n".join(
    ...
    @staticmethod
    def _compute_efficiency_score(speaker_stats: list[dict], llm_score: float, transcript: Any) -> float:
