...
## 阶段一：项目骨架初始化
- [x] 确定项目名称为 OmniSync AI
- [x] 创建新的工作根目录 `omnisync-ai`
- [x] TODO 1: 初始化项目骨架与统一依赖。合并两边的 Python 依赖环境，确保核心音视频处理与 LangGraph 环境相互兼容。
- [x] TODO 2: 建立标准化的基础目录结构（如 `api`, `core`, `agents`, `services` 等）。
- [ ] TODO 2.5: Miniconda 环境标准化。输出标准的 `conda create` 运行指令或 `environment.yml`，固化 ffmpeg 和底层科学计算库的依赖版本，体现系统的标准交付能力。

## 阶段二：核心模块抽离与迁移
- [ ] TODO 3: 提取多智能体中枢。将 `multi-agent-meeting-assistant-main/python` 目录下的 LangGraph 工作流逻辑和各个 Agent（Summary, Action, Insight）迁移到新目录。
